...
class QRPaymentSVGImage(svg.SvgPathImage):
    """
    QR Payment SVG image generator.
    ...
    def make_border(self):
        ...
        return ET.Element("path", d=subpaths, id="qr-border", **self.QR_PATH_STYLE)

    def make_text(self):
        """Creates "QR payment" text element"""
        scaled = self._get_scaled_sizes()
        ...
        y_pos = str(
            ...
            + scaled.width
            + (self.FONT_HEIGHT / 4) * scaled.ratio - 1
        )

        text_el = ET.Element("text", style=text_style, x=x_pos, y=y_pos, id="qr-text")
        text_el.text = "1.H QR payment"
        return text_el

    def make_amount(self):
        scaled = self._get_scaled_sizes()
        desc_text_style = "font-size:{size}px;fill:#222222;font-family:Arial;"
        text_style = desc_text_style.format(size=(Decimal("5.2") * scaled.ratio).quantize(Decimal("0.01")))

        x_pos = str(scaled.outside_border + scaled.line_size + 4 * scaled.ratio - 8)
        y_pos = str(
            scaled.outside_border
            + scaled.line_size
            + 2 * scaled.inside_border
            + scaled.width
            + (self.FONT_HEIGHT / 4) * scaled.ratio + 17
        )

        text_el = ET.Element("text", style=text_style, x=x_pos, y=y_pos, id="qr-text")
        text_el.text = self.AMOUNT
        return text_el

    def make_title(self):
        scaled = self._get_scaled_sizes()
        desc_text_style = "font-size:{size}px;fill:#222222;font-family:Arial;"
        text_style = desc_text_style.format(size=(Decimal(str(min(75 / len(self.TITLE), 7))) * scaled.ratio).quantize(Decimal("0.01")))
        ...
        y_pos = str(
            ...
            + 2 * scaled.inside_border
            + scaled.width
            + (self.FONT_HEIGHT / 4) * scaled.ratio - 103
        )

        text_el = ET.Element("text", style=text_style, x=x_pos, y=y_pos, id="qr-text")
        text_el.text = self.TITLE
        return text_el

    def _svg(self, view_box=None, **kwargs):
        scaled = self._get_scaled_sizes()
        ...
        svg_el.append(self.make_amount())
        svg_el.append(self.make_title())

        # update size of the SVG element
        svg_el.attrib["height"] = str(self.units(h_pixels + 400))
