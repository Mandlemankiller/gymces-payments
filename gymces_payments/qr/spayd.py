...
import re
import qrcode
from gymces_payments.qr.svg import QRPaymentSVGImage


def _format_item_string(item, name):
    if item:
...
class QRPaymentGenerator:
    ...
    def get_text(self):
        return "SPD*1.0*{ACC}{ALTACC}{AM}{CC}{RF}{RN}{DT}{PT}{MSG}{NT}{NTA}{XPER}{XVS}{XSS}{XKS}{XID}{XURL}".format(
            ...
            AM=self._amount,
            CC=_format_item_string(self.currency, "CC"),
            RF=_format_item_string(self.reference, "RF"),
            RN=_format_item_string(self.recipient_name, "RN"),
            DT=self._due_date,
            PT=_format_item_string(self.payment_type, "PT"),
            MSG=_format_item_string(self.message, "MSG"),
            NT=_format_item_string(self.notification_type, "NT"),
            NTA=_format_item_string(self.notification_address, "NTA"),
            XPER=_format_item_string(self.x_per, "X-PER"),
            XVS=_format_item_string(self.x_vs, "X-VS"),
            XSS=_format_item_string(self.x_ss, "X-SS"),
            XKS=_format_item_string(self.x_ks, "X-KS"),
            XID=_format_item_string(self.x_id, "X-ID"),
            XURL=_format_item_string(self.x_url, "X-URL"),
        ).rstrip("*")

    def make_image(self, amount, title, border=2, box_size=12, error_correction=qrcode.constants.ERROR_CORRECT_M):
        QRPaymentSVGImage.AMOUNT = amount
        QRPaymentSVGImage.TITLE = title
        qr = qrcode.QRCode(
            version=None,
            error_correction=error_correction,
            image_factory=QRPaymentSVGImage,
            border=border,
            box_size=box_size
        )
